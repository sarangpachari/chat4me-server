...
exports.createGroup = async (req, res) => {
  try {
    ...
    const groupMembersArray = groupMember.map(
...
exports.getUserGroups = async (req, res) => {
  try {
    ...
    const groups = await Group.find({
      $or: [{ createdBy: id }, { groupMembers: id }],
    });
  ...
  try {
    ...
    if (id !== group.createdBy.toString()) {
      return res
        .status(400)
        .json({
          message: "Unauthorized: Only the group creator can delete the group",
        });
    }
  ...
  try {
    ...
    if (id !== group.createdBy.toString()) {
      return res
        .status(403)
        .json({
          message: "Unauthorized: Only the group creator can clear messages",
        });
    }
